 ...
 * 3、Bitmap形式
 * 这三种形式的区别: 文件形式和流的形式对图片体积大小并没有影响,也就是说,
 * 如果你手机SD卡上的如果是100K,那么通过流的形式读到内存中,也一定是占100K的内存,
...
//       fos.close(); 
	 ...
	 * double mid = b.length / 1024;  将字节换成KB
	 * 
	 * @param bitmap
	 * @return
	 */
...
//       options = calculateInSampleSize(options, reqWidth, reqHeight);
    ...
    public static Bitmap getBitmapFromStream(InputStream is, int reqWidth,
                                             int reqHeight) {
        byte[] data = IOFormat.inputStream2Bytes(is);
        //将输入流转为byte数组，然后调用从byte数组中获得图片的方法
    ...
    /**
     * 返回图片的缩略图. 缩略图的大小定义的维度
     * 这种方法不是线程安全的,应该只在UI线程上调用。
     *
     * @param bitmap  
     * @param context
     * @return 图片的缩略图不能创建缩略图。
     */
     ...
     */
    public static Bitmap scale(Bitmap bitmap, double newWidth, double newHeight) {
        // 记录bitmap的宽高
     ...
     * @param scaleY ：纵向缩放比例
     */
    public static Bitmap scaleflot(Bitmap src, float scaleX, float scaleY) {
        Matrix matrix = new Matrix();
    ...
    public static Bitmap combineImagesToSameSize(Bitmap bgd, Bitmap fg) {
        ...
        if (fg.getWidth() != width && fg.getHeight() != height) {
            fg = scale(fg, width, height);
        }
        if (bgd.getWidth() != width && bgd.getHeight() != height) {
            bgd = scale(bgd, width, height);
        }
     ...
     * 生成水印图片 水印在右下角
     *
     * @param src       你想要加工的位图对象
     * @param watermark 上面的水印src
     * @return 返回一个位图对象,如果参数长度为0,返回null
     */
     ...
     *
     * @param context    
     * @param sentBitmap 要转换的图
     * @param radius     想要的半径, 0 < r < 25
     * @return 生成的模糊图像
     */
    ...
    public static Bitmap blur(Context context, Bitmap sentBitmap, int radius) {
        ...
        if (radius < 0) {
            radius = 0;
            if (DEBUG) {
                Log.w(TAG, "radius must be 0 < r < 25 , forcing radius=0");
            }
        } else if (radius > 25) {
            radius = 25;
            if (DEBUG) {
                Log.w(TAG, "radius must be 0 < r < 25 , forcing radius=25");
            }
        }
