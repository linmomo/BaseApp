...
import com.sixun.basework.logutils.utils.ArrayUtil;
import com.sixun.basework.logutils.utils.SystemUtil;
import com.sixun.basework.logutils.utils.Util;
...
public final class Logger implements Printer {
	...
	private String generateTag(StackTraceElement caller) {
		...
		stackTrace = stackTrace.substring(stackTrace.lastIndexOf('('),
				stackTrace.length());
		String tag = "%s%s.%s%s";
		String callerClazzName = caller.getClassName();// 返回类名
		callerClazzName = callerClazzName.substring(callerClazzName
				.lastIndexOf(".") + 1);
		String Prefix = TextUtils.isEmpty(KL.configTagPrefix) ? ""
				: KL.configTagPrefix;
...
//		}
		...
		try {
			if (json.startsWith("{")) {
				JSONObject jsonObject = new JSONObject(json);
				message = jsonObject.toString(indent);
			} else if (json.startsWith("[")) {
				JSONArray jsonArray = new JSONArray(json);
				message = jsonArray.toString(indent);
			} else {
				message = json;
			}
		} catch (JSONException e) {
		...
		Util.printLine(element.getClassName(), true);
		message = LINE_SEPARATOR + message;
		String[] lines = message.split(LINE_SEPARATOR);
		for (String line : lines) {
			d(element, "║ " + line);
		}
		Util.printLine(element.getFileName(), false);
	}
}
